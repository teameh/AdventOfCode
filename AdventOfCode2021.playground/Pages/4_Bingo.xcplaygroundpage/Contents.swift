...
func firstPuzzle(input: String, bingoNumbers: [Int]) -> Int {
    let boards = makeBoards(input: input)
    return bingoNumbers
        .enumerated()
        .compactMap { offset, number -> Int? in
            let calledBingoNumbers = bingoNumbers.prefix(offset + 1)
            let sumOfUnmarkedNumbers = boards
                .first { $0.hasBingo(bingoNumbers: Array(calledBingoNumbers)) }?
                .boardNumbers
                .reduce([], +)
                .filter { !calledBingoNumbers.contains($0) }
                .reduce(0, +)
            return sumOfUnmarkedNumbers.map { $0 * number }
        }
        .first!
}

func secondPuzzle(input: String, bingoNumbers: [Int]) -> Int {
    let boards = makeBoards(input: input)
    return bingoNumbers
        .enumerated()
        .reduce((boards: boards, scores: Array<Int>())) { (partialResult, iteration) in
            let (offset, number) = iteration
            let (boardsLeft, scores) = partialResult
            let calledBingoNumbersUntilNow = bingoNumbers.prefix(offset + 1)

            let newWinners = boardsLeft
                .filter { $0.hasBingo(bingoNumbers: Array(calledBingoNumbersUntilNow)) }

            let newScores = newWinners.map { board in
                number * board
                    .boardNumbers
                    .reduce([], +)
                    .filter { !calledBingoNumbersUntilNow.contains($0) }
                    .reduce(0, +)
            }

            return (
                boardsLeft.filter { !newWinners.contains($0) },
                scores + newScores
            )
        }
        .1
