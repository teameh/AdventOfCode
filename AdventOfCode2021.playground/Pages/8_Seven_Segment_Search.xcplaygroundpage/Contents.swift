...
extension Reading {
    ...
    func isEqual(to: Reading) -> Bool {
        count == to.count && sorted() == to.sorted()
    }
}
...
struct Entry {
    ...
    var easyOuputDigits: [Int] {
        output.compactMap { $0.easyDigit }
    }

    func makeInputDigitDict() -> [Reading: Int] {
        var inputsToGo = input

        var result: [Reading: Int] = input.reduce(into: [:], { res, reading in
            if let easyDigit = reading.easyDigit {
                res[reading] = easyDigit
            }
        })
        inputsToGo = inputsToGo.filter { $0.easyDigit == nil }

        let reading1 = result.first { $0.value == 1 }!.key

        let reading3 = inputsToGo.first { reading in
            reading.count == 5 && reading1.allSatisfy { reading.contains($0) }
        }!
        result[reading3] = 3
        inputsToGo = inputsToGo.filter { $0 != reading3 }

        let reading9 = inputsToGo.first { reading in
            reading.count == 6 && reading3.allSatisfy { reading.contains($0) }
        }!
        result[reading9] = 9
        inputsToGo = inputsToGo.filter { $0 != reading9 }

        let reading5 = inputsToGo.first { reading in
            reading.count == 5 && reading.allSatisfy { reading9.contains($0) }
        }!
        result[reading5] = 5
        inputsToGo = inputsToGo.filter { $0 != reading5 }

        let reading2 = inputsToGo.first { $0.count == 5 }!
        result[reading2] = 2
        inputsToGo = inputsToGo.filter { $0 != reading2 }

        let reading6 = inputsToGo.first { reading in
            reading.count == 6 && reading5.allSatisfy { reading.contains($0) }
        }!
        result[reading6] = 6
        inputsToGo = inputsToGo.filter { $0 != reading6 }

        let reading0 = inputsToGo.first!
        result[reading0] = 0
    ...
    var outputDigits: [Int] {
        let inputDigitDict = makeInputDigitDict()
        return output.map { outputReading in
            inputDigitDict
                .first { (reading, digit) in
                    reading.isEqual(to: outputReading)
                }!
                .value
        }
    }
...
func parseInput(input: String) -> [Entry] {
    return input
        ...
        .map {
            ...
            return Entry(
                input: Array(parts[0].split(separator: " ").map(Reading.init)),
                output: Array(parts[1].split(separator: " ").map(Reading.init))
            )
        }
}
...
func puzzle1(input: String) -> Int {
    parseInput(input: input)
        .reduce(0) { $0 + $1.easyOuputDigits.count }
}

func puzzle2(input: String) -> Int {
     return parseInput(input: input)
        .map { entry in
            let digitsString = entry
                .outputDigits
                .map { "\($0)" }
                .reduce("", +)
