...
if calculateResultPuzzle1(input: testInput) != 150 {
...
func calculateResultPuzzle2(input: [Command]) -> Int {
    let (horizontal, vertical, _) = input
        .reduce(into: (horizontal: 0, depth: 0, aim: 0)) { partialResult, comand in
            switch comand {
            case .forward(let amount):
                partialResult.0 += amount
                partialResult.1 += (amount * partialResult.2)
            case .down(let amount):
                partialResult.2 += amount
            case .up(let amount):
                partialResult.2 -= amount
            }
